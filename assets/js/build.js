...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function registerTransitionsFromHelper(el, modifiers, stage) {
    ...
    let easing2 = `cubic-bezier(0.4, 0.0, 0.2, 1)`;
    if (transitioningIn) {
  ...
  var getRandomMinMaxInt = (min, max) => {
    return getRandomInt(max - min) + min;
  };
  var randomRotation = () => {
    return getRandomInt(180);
  };
  var bound = (value, min, max) => {
    return Math.max(Math.min(value, max), min);
  };
  var calculatePercentageThrough = (from, to, percentageComplete) => {
  ...
  var maxDistance = 200;
  var maxDistanceTravelled = Math.sqrt(maxDistance ** 2 * 2);
  var Actor = class {
    constructor({ colour, rotation, size: size2, x, y, maxX, minX, maxY, minY, minSize, maxSize }) {
      this.colour = colour;
      this.position = {
        opacity: 0,
        rotation: rotation || randomRotation(),
        size: size2 || getRandomMinMaxInt(minSize, maxSize),
    ...
    update(delta) {
      ...
      if (!this.nextPosition || this.nextPosition.duration <= this.nextPosition.elapsed) {
        ...
        this.nextPosition = {
          ...this.position,
          duration: this.position.opacity === 0 ? getRandomMinMaxInt(2e3, 5e3) : getRandomMinMaxInt(800, 2e3),
          elapsed: 0,
          newOpacity: 1,
      ...
      const { opacity, newOpacity, x, newX, y, newY, size: size2, newSize, rotation, newRotation } = this.nextPosition;
      ...
      this.position = {
        ...
        size: calculatePercentageThrough(size2, newSize, percentageComplete),
        rotation: calculatePercentageThrough(rotation, newRotation, percentageComplete)
      };
    }
  };

  // assets/js/hexagon.js
  ...
  var a = 2 * Math.PI / 6;
  var Hexagon = class extends Actor {
    draw(ctx) {
      const { x, y, size: size2, rotation, opacity } = this.position;
      const percentageOfMaxSize = size2 / this.maxSize;
      ...
      ctx.beginPath();
      for (var i = 0; i < 6; i++) {
        ctx.lineTo(x + size2 * Math.cos(a * i + radians), y + size2 * Math.sin(a * i + radians));
      }
      ctx.closePath();
      ctx.stroke();
    }
  ...
  var Stage = class {
    ...
    draw() {
      ...
      this.actors.sort((a2, b) => a2.depth() - b.depth()).forEach((actor) => {
